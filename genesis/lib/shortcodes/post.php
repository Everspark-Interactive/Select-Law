...
function genesis_post_author_shortcode( $atts ) {
    ...
    $output .= $atts['before'];
    $output .= sprintf( '<span %s>', genesis_attr( 'entry-author-name' ) ) . '<a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '">' . esc_html( $author ) . '</a></span>'; // Make author clickable
    $output .= $atts['after'];
    $output .= '</span>';
} else {
    $output = sprintf( '<span class="author vcard">%2$s<span class="fn">%1$s</span>%3$s</span>', '<a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '">' . esc_html( $author ) . '</a>', $atts['before'], $atts['after'] ); // Make author clickable
}
